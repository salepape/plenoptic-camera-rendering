	...
	else if (opts.Language_Version >= 350)
	{
      ...
      New->Fish_ZOffset = 0;
		...
		EXPECT
			...
			END_CASE
         CASE (STEREOPANORAMA_TOKEN)
            New->Type = STEREOPANORAMA_CAMERA;
         END_CASE
         CASE (STEREOSPHERICAL_TOKEN)
            New->Type = STEREOSPHERICAL_CAMERA;
         END_CASE
         CASE (STEREOFISHEYE1_TOKEN)
            New->Type = STEREOFISHEYE1_CAMERA;
         END_CASE
         CASE (STEREOFISHEYE2_TOKEN)
            New->Type = STEREOFISHEYE2_CAMERA;
         END_CASE
         CASE (OFFSETFISHEYE_TOKEN)
            New->Type = OFFSETFISHEYE_CAMERA;
         END_CASE
		...
		switch(New->Type)
		{
			...
			// pdb - don't fully appreciate what is going on here
...
bool Parse_Camera_Mods(CAMERA *New)
{
	...
	EXPECT_ONE
      ...
      END_CASE
      CASE (FISHXOFFSET_TOKEN)
         New->Fish_XOffset = Parse_Float();
      END_CASE
      CASE (FISHYOFFSET_TOKEN)
         New->Fish_YOffset = Parse_Float();
      END_CASE
      CASE (FISHZOFFSET_TOKEN)
         New->Fish_ZOffset = Parse_Float();
      END_CASE
