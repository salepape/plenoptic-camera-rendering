...
*/
inline void VRotateX(VECTOR q,const VECTOR p,DBL theta)
{
   q[X] = p[X];
   q[Y] = p[Y] * cos(theta) + p[Z] * sin(theta);
   q[Z] = -p[Y] * sin(theta) + p[Z] * cos(theta);
}
inline void VRotateY(VECTOR q,const VECTOR p,DBL theta)
{
   q[X] = p[X] * cos(theta) - p[Z] * sin(theta);
   q[Y] = p[Y];
   q[Z] = p[X] * sin(theta) + p[Z] * cos(theta);
}
inline void VRotateZ(VECTOR q,const VECTOR p,DBL theta)
{
