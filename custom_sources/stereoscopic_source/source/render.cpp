  ...
  switch (Frame.Camera->Type)
  {
    ...
    case STEREOPANORAMA_CAMERA:
      ...
      // phi = atan(y0 * Frame.Screen_Height * M_PI / Frame.Screen_Width);
		// Change to support other angles besider 360
      theta = x0 * Frame.Camera->Angle * M_PI_180;
      phi = atan(y0 * Frame.Screen_Height * M_PI * (Frame.Camera->Angle/360) / Frame.Screen_Width);
    ...
    /*
     * pdb - Stereoscopic fisheye camera #1
     * Currently only 180 degree fisheyes supported
     * Image dimensions should be equal for square pixels, width = heigth
     * This is a front stereofisheye only -- same as offset fisheye solution?
     */
    case STEREOFISHEYE1_CAMERA:

      // Normalise projection plane coordinates
      x0 = 2.0 * x / (DBL)Frame.Screen_Width - 1.0; // -1 .. 1
      y0 = 2.0 * ((DBL)(Frame.Screen_Height - 1) - y) / (DBL)Frame.Screen_Height - 1.0; // -1 ..1
      ...
      // Fisheye polar coordinates - only support 180 degree at this stage
      theta = atan2(y0,x0); // -pi .. pi
      phi = rad * 90 * M_PI_180;  // 0 .. pi/2

		// Vector to the point on the dome
      vt1[0] = Frame.Camera->Zero_Parallax * sin(phi)*cos(theta);
      vt1[1] = Frame.Camera->Zero_Parallax * sin(phi)*sin(theta);
      vt1[2] = Frame.Camera->Zero_Parallax * cos(phi);
      ...
      // Camera position and direction ray
		vt2[0] = Frame.Camera->Eye_Offset;
		vt2[1] = 0;
		vt2[2] = 0;
		vt1[0] -= vt2[0];
      vt1[1] -= vt2[1];
      vt1[2] -= vt2[2];
		...
		// Primary ray
      VLinComb3(Ray->Initial,vt2[0],vr,vt2[1],vu,vt2[2],vd);
      Ray->Initial[0] += Frame.Camera->Location[0];
      Ray->Initial[1] += Frame.Camera->Location[1];
      Ray->Initial[2] += Frame.Camera->Location[2];
    ...
    /*
     * pdb - Stereoscopic fisheye camera #2
     * Currently only 180 degree fisheyes supported
     * Image dimensions should be equal for square pixels, width = heigth
     * This is the "correct" omnidirectional camera
     */
    case STEREOFISHEYE2_CAMERA:
      ...
      // Camera position and direction ray
      vt2[0] = Frame.Camera->Eye_Offset * sin(theta2);
      vt2[1] = 0;
      vt2[2] = -Frame.Camera->Eye_Offset * cos(theta2);
      vt1[0] -= vt2[0];
      vt1[1] -= vt2[1];
      vt1[2] -= vt2[2];
    ...
    case OFFSETFISHEYE_CAMERA:
      ...
      // Vector to the point on the unit dome
      vt1[0] = sin(phi)*cos(theta);
      vt1[1] = sin(phi)*sin(theta);
      vt1[2] = cos(phi);
      ...
      // Camera position and direction ray
      vt2[0] = Frame.Camera->Fish_XOffset;
      vt2[1] = Frame.Camera->Fish_YOffset;
      vt2[2] = Frame.Camera->Fish_ZOffset;
      vt1[0] -= vt2[0];
      vt1[1] -= vt2[1];
      vt1[2] -= vt2[2];
    ...
    case FISHEYE_CAMERA:
      ...
      if (rad > 1.0) {
		...
		// Could be replaced by phi = atan2(y0,x0);
      if (rad == 0.0) {
        phi = 0.0;
      } else {
        if (x0 < 0.0) {
